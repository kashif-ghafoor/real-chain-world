...
contract("Property", (accounts) => {
  const companyAccount = accounts[0];
  ...
  beforeEach(async () => {
    ...
    const response = await factoryInstance.createPropertyTokenizeRequest(
      property1.address,
    ...
    await factoryInstance.suggestPrice(requestId, suggestedPrice, {
      from: companyAccount,
    });
    await factoryInstance.acceptPrice(requestId, { from: ownerAccount });
    assert.equal(response.logs[0].event, "PriceAccepted", "Wrong event triggered");

    await factoryInstance.createProperty(requestId, { from: companyAccount });
  ...
  describe("Functions", () => {
    describe("buyShare", async () => {
      ...
      it("New holder address should be added to holders", async () => {
        await property1Instance.buyShare(ownerAccount, 2, {
          from: account1,
          value: etherToWei(20),
        });
        let holders = await property1Instance.getHolders();
      ...
      it("Should revert when funds are less than the price", async () => {
        await catchRevert(
          property1Instance.buyShare(ownerAccount, 2, { from: account3, value: etherToWei(10) })
        );
      });
      ...
      it("Should revert when seller owns less tokens than the requested", async () => {
        ...
        await catchRevert(
          property1Instance.buyShare(ownerAccount, 9, { from: account2, value: etherToWei(90) })
        );
      });
      ...
      it("Should revert when requested buyer is not selling", async () => {
        ...
        await catchRevert(
          property1Instance.buyShare(ownerAccount, 2, { from: account1, value: etherToWei(20) })
        );
      });
    });
    ...
    describe("distribute", async () => {
      ...
      it("Holder's revenue should increase", async () => {
        await property1Instance.setMySharesForSale({ from: ownerAccount });
        await property1Instance.buyShare(ownerAccount, 1, {
          from: account1,
          value: etherToWei(10),
        });
        ...
        assert(
          account1RevenueBeforeDistribution < account1RevenueAfterDistribution,
          "Holder's revenue did not increase after distribution"
        );
      });
    ...
    describe("withdraw", async () => {
      ...
      it("Should send revenue amount in to holder", async () => {
        ...
        assert(
          etherBalanceAfterWithdrawal - etherBalanceBeforeWithdrawal > 0,
          etherBalanceAfterWithdrawal
        );
      });
    });
    ...
    describe("circuitBreaker", async () => {
      ...
      it("Should revert if someone tries to buy when circuit is paused", async () => {
        ...
        await catchRevert(
          property1Instance.buyShare(account5, 2, { from: account6, value: etherToWei(20) })
        );
      });
